...
final class CodexAppServerTitleResolverTests: XCTestCase {
    ...
    func testHealthReportsCategoricalTransitionsWithoutPrivateInput() async {
        let now = LockedValue(0.0)
        let calls = LockedValue(0)
        ...
        XCTAssertEqual(health.read(), [.timeout, .unavailable, .healthy])
    ...
    func testProcessRunnerRejectsNearMatchThreadNotLoadedErrors() async throws {
