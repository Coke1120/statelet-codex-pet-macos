...
struct SessionActivityResolvedTitleStore {
    private var titles: [String: String] = [:]
    private var threadIDs: [String: String] = [:]

    mutating func retain(for activityThreads: [String: String]) {
        titles = titles.filter { id, _ in threadIDs[id] == activityThreads[id] }
        threadIDs = threadIDs.filter { id, threadID in activityThreads[id] == threadID }
    }

    mutating func adopt(
        _ resolved: [String: String],
        expectedThreads: [String: String],
        currentThreads: [String: String]
    ) {
        for (id, title) in resolved where currentThreads[id] == expectedThreads[id] {
            titles[id] = title
            threadIDs[id] = expectedThreads[id]
        }
        retain(for: currentThreads)
    }

    func unresolvedTargets(
        activityThreads: [String: String],
        suppliedTitles: [String: String]
    ) -> [String: String] {
        activityThreads.filter { id, threadID in
            suppliedTitles[id] == nil
                && !(threadIDs[id] == threadID && titles[id] != nil)
        }
    }

    func combined(with suppliedTitles: [String: String]) -> [String: String] {
        titles.merging(suppliedTitles) { _, supplied in supplied }
    }

    mutating func removeAll() {
        titles.removeAll(keepingCapacity: false)
        threadIDs.removeAll(keepingCapacity: false)
    }
}
...
enum CodexAppServerResolutionFailure: Error, Equatable {
    case unavailable
    case timeout
