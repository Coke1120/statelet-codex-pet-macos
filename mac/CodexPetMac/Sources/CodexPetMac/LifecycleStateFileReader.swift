...
enum LifecycleStateReadResult: Equatable {
...
final class LifecycleStateFileReader: @unchecked Sendable {
    ...
    init(loader: @escaping @Sendable (URL) -> LifecycleStateReadResult = { url in
        LifecycleStateFileReader.load(url)
    }) {
        self.loader = loader
    }

    func read(_ url: URL, completion: @escaping @Sendable (LifecycleStateReadResult) -> Void) {
        lock.lock()
        generation &+= 1
    ...
    static func load(_ url: URL) -> LifecycleStateReadResult {
        let descriptor = Darwin.open(url.path, O_RDONLY | O_NONBLOCK | O_NOFOLLOW | O_CLOEXEC)
        guard descriptor >= 0 else { return errno == ENOENT ? .missing : .corrupt }
        ...
        guard Darwin.fstat(descriptor, &after) == 0,
              ...
              let state = try? JSONDecoder.codexPet.decode(CurrentState.self, from: data) else {
            return .corrupt
        }
        return .state(state)
    }
