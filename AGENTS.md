...
- **Companion Quick Chat is an explicit user-initiated exception:** the signed installed Codex CLI sends typed messages and selected text context through the user's Codex connection. Preserve ephemeral stdin-only conversation handling, disabled tools/hooks/plugins, bounded output and cancellation, and the model/credential-free-loopback-only config allowlist. Do not put chat text or dictated audio into diagnostics, shared state or persistent history. On-device dictation must not fall back to cloud recognition. See `docs/COMPANION.md`.
